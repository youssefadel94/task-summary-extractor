...
async function assessProgressWithAI(ai, items, changeReport, localAssessments, opts = {}) {
  ...
  if (!parsed) {
    throw new Error('Failed to parse AI progress assessment response as JSON. Try a different model (--model gemini-3.1-pro-preview) or run again — transient API issues can cause this.');
  }
...
function renderProgressMarkdown({ assessments, changeReport, overallSummary, recommendations, meta = {} }) {
  ...
  lines.push(`*Generated by Smart Change Detection (v${pkgVersion})*`);
